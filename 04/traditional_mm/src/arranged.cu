...
int main(int argc, char **argv)
{
    ...
    for (int dim = min_dim; dim <= max_dim; dim += step)
    {
        ...
        {
            if (dim < MaxThreadsPerBlock)
            {
                threadsPerBlock.x = dim;
                threadsPerBlock.y = MaxThreadsPerBlock / dim;
            }
            else
            {
                threadsPerBlock.x = sqrt(MaxThreadsPerBlock);
                threadsPerBlock.y = sqrt(MaxThreadsPerBlock);
            }
            blocksPerGrid.x = ceil(float(dim) / float(threadsPerBlock.x));
            blocksPerGrid.y = ceil(float(dim) / float(threadsPerBlock.y));
        }

        printf("Dim: %d, ThreadsPerBlock: (%d, %d), BlocksPerGrid: (%d, %d)\n", dim, threadsPerBlock.x, threadsPerBlock.y, blocksPerGrid.x, blocksPerGrid.y);
        ...
        saveTimeToFile(dim, load_duration.count(), "csv/load-arranged.csv");
        ...
        saveTimeToFile(dim, calc_duration.count(), "csv/calc-arranged.csv");
        ...
        saveTimeToFile(dim, read_duration.count(), "csv/read-arranged.csv");
        saveTimeToFile(dim, load_duration.count() + calc_duration.count() + read_duration.count(), "csv/total-arranged.csv");
        saveTimeToFile(dim, load_duration.count() + read_duration.count(), "csv/overhead-arranged.csv");
