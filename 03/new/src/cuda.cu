...
__global__ void perElement_MM(float *A, float *B, float *R, int R_rows, int M_dims, int R_cols)
{
...
int main(int argc, char **argv)
{
    ...
    for (int dim = min_dim; dim <= max_dim; dim += step)
    {
        ...
        if (dim * dim > MaxThreadsPerBlock)
        {
            ...
            {
                threadsPerBlock.x = static_cast<int>(sqrt(MaxThreadsPerBlock));
                threadsPerBlock.y = static_cast<int>(sqrt(MaxThreadsPerBlock));
            }
        ...
        perElement_MM<<<blocksPerGrid, threadsPerBlock>>>(gpu_A, gpu_B, gpu_R, dim, dim, dim);
        cudaDeviceSynchronize();
