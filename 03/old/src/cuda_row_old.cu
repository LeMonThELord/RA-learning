...
__global__ void matrixMultiplication(float *A, float *B, float *R, int R_rows, int M_dims, int R_cols)
{
    ...
    for (int i = index; i < R_rows; i += stride)
    {
        ...
        for (int j = 0; j < R_cols; j++)
        {
            ...
            {
                temp[j] += A[i * M_dims + k] * B[k * R_cols + j];
            }
        }
...
int main(int argc, char **argv)
{
    ...
    for (int dim = min_dim; dim <= max_dim; dim += step)
    {
        ...
        saveTimeToFile(dim, load_duration.count(), "csv/load-row-cuda-old-" + to_string(threads_count) + ".csv");
        ...
        saveTimeToFile(dim, calc_duration.count(), "csv/calc-row-cuda-old-" + to_string(threads_count) + ".csv");
        ...
        saveTimeToFile(dim, read_duration.count(), "csv/read-row-cuda-old-" + to_string(threads_count) + ".csv");
        saveTimeToFile(dim, load_duration.count() + calc_duration.count() + read_duration.count(), "csv/total-row-cuda-old-" + to_string(threads_count) + ".csv");
        saveTimeToFile(dim, load_duration.count() + read_duration.count(), "csv/overhead-row-cuda-old-" + to_string(threads_count) + ".csv");
