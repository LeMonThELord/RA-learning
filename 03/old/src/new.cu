...
__global__ void row_MM(float *A, float *B, float *R, int R_rows, int M_dims, int R_cols)
{
    ...
    for (int i = index; i < R_rows; i += stride)
    {
        for (int j = 0; j < R_cols; j++)
        {
            ...
            {
                R[i * R_cols + j] += A[i * M_dims + k] * B[k * R_cols + j];
            }
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    for (int dim = min_dim; dim <= max_dim; dim += step)
    {
        ...
        if (dim * dim > MaxThreadsPerBlock)
        {
            ...
            // 调整线程块大小
            threadsPerBlock = MIN(threadsPerBlock, dim);
        ...
        row_MM<<<blocksPerGrid, threadsPerBlock>>>(gpu_A, gpu_B, gpu_R, dim, dim, dim);
        cudaDeviceSynchronize();
