...
int main(int argc, char **argv)
{
    ...
    for (int dim = min_dim; dim <= max_dim; dim += step)
    {
        ...
        if (DEBUG)
        {
            pprintMatrix(A, dim, dim, "improved A");
            pprintMatrix(B, dim, dim, "improved B");
        }
        ...
        if (DEBUG)
        {
            pprintMatrix(R, dim, dim, "improved R");
        ...
        saveTimeToFile(dim, duration.count(), "csv/total-improved.csv");
