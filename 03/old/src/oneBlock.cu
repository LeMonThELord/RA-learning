...
__global__ void MatMult3d(float *A, float *B, float *R, int R_rows, int M_dims, int R_cols)
{
...
__global__ void MatMultRow(float *A, float *B, float *R, int R_rows, int M_dims, int R_cols)
{
...
int main(int argc, char **argv)
{
    ...
    for (int dim = min_dim; dim <= max_dim; dim += step)
    {
        ...
        MatMult3d<<<blocksPerGrid, threadsPerBlock>>>(gpu_A, gpu_B, gpu_R, dim, dim, dim);
        cudaDeviceSynchronize();
        ...
        saveTimeToFile(dim, calc_duration.count(), "csv/calc-cuda.csv");
